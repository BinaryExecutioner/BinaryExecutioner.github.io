...
Consolidated various users and ran a name masher script to generate a list of possible usernames from a personâ€™s first and last name:
...
After obtaining the AS-REP hash, attempt to crack it usingÂ HashcatÂ with mode 18200, which is specifically designed for Kerberos AS-REP hashes. Here, I used the popularÂ rockyou.txtÂ wordlist to brute-force the password:
```bash
...
Using the cracked credentials, logged into the machine withÂ **Evil-WinRM**Â to gain an interactive PowerShell session:
...
In addition to fsmith, thereâ€™s an additional user named svc_loanmgr
