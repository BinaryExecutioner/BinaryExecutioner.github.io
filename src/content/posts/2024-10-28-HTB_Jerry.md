...
A service named ÄúApache TomcatÄù running on the port 8080. The http-open-proxy script indicates that the server might be misconfigured as an open proxy.
...
Tried accessing ÄúServer statusÄù, which returned a pop-up asking for credentials to access the page as shown below.

![image.png](/assets/img/Jerry/image%201.png)

Brute-forced few known credentials found on [google](https://stackoverflow.com/questions/3829513/what-is-the-default-username-and-password-in-tomcat). Username:Password- admin:admin worked for logging into ÄúServer StatusÄù, ÄúManager AppÄù,ÄùHost ManagerÄù

Found some interesting information on the webpage ÄúHost ManagerÄù 

![image.png](/assets/img/Jerry/image%202.png)

With the credentials obtained tomcat:s3cret (Username:Password), logged into ÄúTomcat Web Application ManagerÄù on an another Private tab. 
...
Generating .war payloads using ÄúmsfvenomÄù
...
Accessing the file via web browser at ÄútestÄù location resulted in obtaining a reverse shell.
...
The flags are located at AdministratorÄôs Desktop
